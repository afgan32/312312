...
local THEME = {
    ...
    -- Transparency levels
    GlassAlpha       = 0.72,   -- main panel transparency
    EdgeAlpha        = 0.45,
    ShineAlpha       = 0.55,
    OverlayAlpha     = 0.60,
...
function Util.AddShadow(parent, size, transparency)
    -- Outer glow via ImageLabel trick
...
function Util.GlassPanel(parent, size, position, name, zindex)
    ...
    -- Inner shine highlight (top edge)
...
-- Mouse-based glint animation (simulates gyroscope on PC)
local function AttachGlintToMouse(panel, glintOrb)
    ...
    connection = RunService.RenderStepped:Connect(function()
        ...
        -- Opacity pulse based on distance to center
...
function LiquidGlass:Notify(title, message, notifType, duration)
    ...
    icon.Text              = ({info="ℹ",success="✓",error="✕",warning="⚠"})[notifType] or "ℹ"
...
local UI  = LiquidGlass.new("⬡ LiquidGlass", "visionOS Inspired Theme")
UI:SetKeybind(Enum.KeyCode.RightControl)

-- ── TAB 1: Combat ───────────────────────────────────────────
local Combat = UI:AddTab("Combat", "⚔")
Combat:AddSection("Aimbot")
Combat:AddToggle("Aimbot", "Auto-aim at enemies", false, function(v)
    print("Aimbot:", v)
end)
Combat:AddSlider("FOV", "Field of view radius", 10, 360, 120, function(v)
    print("FOV:", v)
end)
Combat:AddSlider("Smoothness", "Aim smoothness", 1, 100, 30, function(v)
    print("Smooth:", v)
end)
Combat:AddSection("Misc")
Combat:AddButton("Kill Aura", "Damages all nearby players", function()
    print("Kill Aura executed!")
end)
Combat:AddKeybind("Aimbot Key", "Hold to activate", Enum.KeyCode.E, function(k)
    print("New key:", k)
end)

-- ── TAB 2: Visuals ──────────────────────────────────────────
local Visuals = UI:AddTab("Visuals", "◈")
Visuals:AddSection("ESP")
Visuals:AddToggle("Player ESP", "Show player boxes", true, function(v)
    print("ESP:", v)
end)
Visuals:AddColorPicker("ESP Color", "Color of ESP lines",
    Color3.fromRGB(100, 200, 255), function(c)
    print("Color:", c)
end)
Visuals:AddToggle("Tracers", "Draw lines to players", false, function(v)
    print("Tracers:", v)
end)
Visuals:AddDropdown("ESP Style", "Visual style preset",
    {"Box", "Corner Box", "Circle", "Skeleton"}, "Box", function(v)
    print("ESP Style:", v)
end)
...
Settings:AddTextbox("Config Name", "Save/load profile name", "MyConfig", function(text, enter)
    if enter then print("Config:", text) end
end)
Settings:AddButton("Save Config", "Save current settings", function()
    UI:Notify("Saved!", "Config saved successfully.", "success", 3)
end)
Settings:AddButton("Load Config", "Load saved settings", function()
    UI:Notify("Loaded!", "Config loaded.", "info", 3)
end)
Settings:AddSection("Info")
Settings:AddLabel("LiquidGlass UI v1.0", "accent")
Settings:AddLabel("Made with Apple visionOS style", "muted")

-- Test notifications
task.delay(1, function()
    UI:Notify("Welcome!", "LiquidGlass UI loaded. Press RCtrl to toggle.", "success", 5)
end)
